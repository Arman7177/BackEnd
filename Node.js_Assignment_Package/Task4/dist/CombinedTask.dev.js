...
var inputPath = 'jsonFile.json';
var fullPath = path.resolve(inputPath);
var jsonFile = JSON.parse(fs.readFileSync('jsonFile.json', 'utf8'));
console.log(jsonFile); // { name: 'Arman', age: 25, isStudent: true }

jsonFile.timestamp = new Date().toString();
var updatedJsonText = JSON.stringify(jsonFile, null, 2);
...
var outputDir = path.join(parsed.dir, 'output');

if (!fs.existsSync(outputDir)) {
  fs.mkdirSync(outputDir);
}

var outputPath = path.join(outputDir, parsed.base);
...
var buffer = Buffer.from(updatedJsonText, 'utf8');
...
var restoreText = buffer.toString('utf8');
